...
#define SVC_EEP_MAX_BUF_SIZE (SVC_EEP_MAIN_PAGE_SIZE + 4U)

/* ── Static RAM buffers ── */

static uint8_t s_eep_main_mirror[SVC_EEP_MAIN_TOTAL_SIZE];
...
static void svc_eep_clear_page_dirty(const svc_eep_device_t *dev,
                                     svc_eep_size_t page_index);
static svc_eep_size_t svc_eep_addr_to_page(const svc_eep_device_t *dev,
                                           svc_eep_addr_t addr);
static dev_err_t svc_eep_i2c_read(const svc_eep_device_t *dev,
                                  svc_eep_addr_t addr,
                                  uint8_t *data,
                                  svc_eep_size_t length);
static dev_err_t svc_eep_i2c_write_page(const svc_eep_device_t *dev,
                                        svc_eep_addr_t addr,
                                        const uint8_t *data,
                                        svc_eep_size_t length);
static dev_err_t svc_eep_wait_write_cycle(const svc_eep_device_t *dev);
                                  ...
                                  svc_eep_size_t length)
{
    ...
    if ((dev->mem_addr_size == SVC_EEP_MEM_ADDR_SIZE_8BIT) ||
        (dev->mem_addr_size == SVC_EEP_MEM_ADDR_SIZE_16BIT))
    {
        dev_i2c_mem_addr_size_t i2c_addr_size;

        i2c_addr_size = (dev->mem_addr_size == SVC_EEP_MEM_ADDR_SIZE_8BIT)
        ...
        result = dev_i2c_mem_read(dev->i2c_bus,
                                  ...
                                  data,
                                  (uint16_t)length,
                                  SVC_EEP_CFG_ACK_POLL_TIMEOUT_MS);
    }
    else
    {
        ...
        if (dev->mem_addr_size == SVC_EEP_MEM_ADDR_SIZE_24BIT)
        {
        ...
        else /* SVC_EEP_MEM_ADDR_SIZE_32BIT */
        {
                                        ...
                                        svc_eep_size_t length)
{
    ...
    else
    {
        ...
        uint8_t buf[SVC_EEP_MAX_BUF_SIZE];
        uint8_t addr_len;
...
#if (SVC_EEP_CFG_ACK_POLLING_ENABLED == DEV_ON)
    {
        uint32_t elapsed_ms = 0U;
        dev_err_t probe_result;

        while (elapsed_ms < SVC_EEP_CFG_ACK_POLL_TIMEOUT_MS)
        {
...
static void svc_eep_load_defaults(const svc_eep_device_t *dev)
{
    ...
    {
        svc_eep_size_t page;
        svc_eep_size_t page_count = dev->total_size / dev->page_size;
...
dev_err_t svc_eep_init(void)
{
     ...
     * s_initialized (e.g. svc_eep_read_all) can operate during init. */
...
dev_err_t svc_eep_shutdown(void)
{
...
#if (SVC_EEP_CFG_AUTO_FLUSH_ON_SHUTDOWN == DEV_ON)
    {
        for (i = 0U; i < SVC_EEP_CFG_MAX_DEVICES; i++)
        {
            ...
            {
#endif
                result = svc_eep_flush(s_devices[i].eep_id);
                if (result != DEV_OK)
                {
                    if (first_error == DEV_OK)
                    {
...
dev_err_t svc_eep_read_all(svc_eep_id_t eep_id)
{
...
#if (SVC_EEP_CFG_MIRROR_ENABLED == DEV_ON)
    {
        ...
        result = svc_eep_i2c_read(dev, 0U,
                                  dev->mirror,
                                  dev->total_size);
        if (result != DEV_OK)
        {
...
{
    const svc_eep_device_t *dev;
    svc_eep_size_t offset;
    svc_eep_size_t remaining;
    svc_eep_size_t chunk;
    dev_err_t result;
    ...
    while (remaining > 0U)
    {
        ...
        result = svc_eep_i2c_write_page(dev, offset,
        ...
        result = svc_eep_wait_write_cycle(dev);
        if (result != DEV_OK)
        {
            return result;
        }
...
#if (SVC_EEP_CFG_CRC_ENABLED == DEV_ON)
    {
        ...
        {
            svc_eep_size_t crc_chunk;
            svc_eep_addr_t crc_addr = SVC_EEP_LAYOUT_CRC_OFFSET;

            crc_chunk = dev->page_size - (crc_addr % dev->page_size);
            if (crc_chunk > SVC_EEP_LAYOUT_CRC_SIZE)
            {
                crc_chunk = SVC_EEP_LAYOUT_CRC_SIZE;
            }

            result = svc_eep_i2c_write_page(dev, crc_addr,
...
dev_err_t svc_eep_flush(svc_eep_id_t eep_id)
{
    ...
    for (page_index = 0U; page_index < page_count; page_index++)
    {
        if (svc_eep_is_page_dirty(dev, page_index))
        {
            ...
            result = svc_eep_i2c_write_page(dev, page_addr,
                             ...
                             svc_eep_size_t length)
{
    ...
    /* Use SVC_EEP_MAIN as the default device */
    dev = svc_eep_find_device(SVC_EEP_MAIN);
                             ...
                             svc_eep_size_t length)
{
    ...
    {
        start_page = svc_eep_addr_to_page(dev, addr);
        end_page   = svc_eep_addr_to_page(dev, addr + length - 1U);
