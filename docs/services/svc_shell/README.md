# svc_shell — Lightweight UART Command Shell

## 1. Overview

A polling-based command shell that runs over a single UART. Call `svc_shell_handle()` in your main loop — it reads incoming bytes, echoes them, handles backspace, detects Enter, parses the command line, finds the matching command, and calls your callback.
...
root: explain
svc_shell is a small UART command shell.
...
root:
```
...
User types in terminal  →  UART RX interrupt stores bytes in dev_ringbuf
                            ↓
svc_shell_handle()       →  reads one byte via dev_uart_read()
                            ↓
                            ...
                            callback writes output via svc_shell_write_line()
```
...
Add it anywhere in your application code (or in `svc_shell_commands.c`):

```c
#include "svc_shell.h"        /* for svc_shell_write_line, svc_shell_arg_to_u8 */
...
static dev_err_t my_cmd_led(uint8_t argc, char *argv[])
{
    ...
    if (argc != 2U) {
        svc_shell_write_line("Usage: led <0|1>");
        return DEV_ERR_INVALID_ARG;
    }

    /* Parse argv[1] as unsigned 8-bit integer */
    err = svc_shell_arg_to_u8(argv[1], &value);
    if (err != DEV_OK) {
        svc_shell_write_line("Error: expected 0 or 1");
        return err;
    }
...
Open `services/svc_shell/src/svc_shell_commands.c`. Add one line to `g_svc_shell_commands[]`:
...
Runtime commands can be added at runtime without modifying `svc_shell_commands.c`. Useful when a module wants to register its own shell commands during its init.

### Enable the feature

```c
#define SVC_SHELL_CFG_RUNTIME_COMMAND_ENABLED (1U)
...
static dev_err_t cmd_adc(uint8_t argc, char *argv[]) {
    (void)argc; (void)argv;
    return svc_shell_write_line("ADC command called");
}

static const svc_shell_cmd_t adc_cmd = {
...
void app_init(void) {
    svc_shell_init(DEV_UART_CONSOLE);
    svc_shell_register_command(&adc_cmd);   /* register at runtime */
}
```
...
```c
svc_shell_unregister_command("adc");              /* remove one */
svc_shell_unregister_all_runtime_commands();      /* remove all */
uint16_t n = svc_shell_get_runtime_command_count(); /* count */
```
...
- Runtime table cleared on `svc_shell_init()`
...
/* Usage: if (svc_shell_arg_is_equal(argv[1], "on")) { ... } */
...
{
    if (argc == 2U && svc_shell_arg_is_equal(argv[1], "read")) {
        uint32_t value;
        /* Read sensor here */
        return DEV_OK;
    }

    if (argc == 3U && svc_shell_arg_is_equal(argv[1], "write")) {
        uint8_t val;
        dev_err_t e = svc_shell_arg_to_u8(argv[2], &val);
        if (e != DEV_OK) { svc_shell_write_line("Usage: sensor write <0-255>"); return e; }
        /* Write sensor here */
        return DEV_OK;
    }

    svc_shell_write_line("Usage: sensor <read|write> [value]");
    return DEV_ERR_INVALID_ARG;
}
```
...
All configuration macros are in `services/svc_shell/include/svc_shell_cfg.h`. You only need to edit this one file.
...
#define SVC_SHELL_CFG_ECHO_ENABLED            (1U)

/* Print "root:" prompt after Enter */
#define SVC_SHELL_CFG_PROMPT_ENABLED          (1U)

/* Use \r\n for newlines (required for most terminals) */
#define SVC_SHELL_CFG_CRLF_ENABLED            (1U)

/* Handle Backspace key to delete last character */
#define SVC_SHELL_CFG_BACKSPACE_ENABLED       (1U)

/* Support quoted arguments like cmd "hello world" */
#define SVC_SHELL_CFG_QUOTE_PARSE_ENABLED     (1U)

/* Validate init state before each API call */
#define SVC_SHELL_CFG_RUNTIME_CHECK_ENABLED   (1U)
```
...
```c
#define SVC_SHELL_CFG_DEFAULT_PROMPT          "gach>"
```
...
|----------------|-------------|
| Longer command lines | Increase `SVC_SHELL_CFG_MAX_LINE_LENGTH` |
| More arguments per command | Increase `SVC_SHELL_CFG_MAX_ARGS` |
| Disable echo (password input) | Set `SVC_SHELL_CFG_ECHO_ENABLED` to `0U` |
| Disable backspace | Set `SVC_SHELL_CFG_BACKSPACE_ENABLED` to `0U` |
| Different prompt | Change `SVC_SHELL_CFG_DEFAULT_PROMPT` |
| LF-only newlines (some terminals) | Set `SVC_SHELL_CFG_CRLF_ENABLED` to `0U` |
...
dev_err_t svc_shell_write("Processing...");

/* Write text followed by newline (\r\n or \n depending on CRLF setting) */
dev_err_t svc_shell_write_line("Done.");

/* Write raw binary data of specified length */
dev_err_t svc_shell_write_data(&byte, 1U);
```
...
```c
dev_err_t svc_shell_init(dev_uart_id_t uart_id);       /* Init shell + UART */
dev_err_t svc_shell_deinit(void);                       /* Deinit shell */
dev_err_t svc_shell_handle(void);                       /* Process one iteration */
dev_err_t svc_shell_print_prompt(void);                 /* Print prompt now */
dev_err_t svc_shell_write(const char *text);            /* Write string */
dev_err_t svc_shell_write_line(const char *text);       /* Write string + newline */
dev_err_t svc_shell_write_data(const uint8_t *d, uint16_t len); /* Write raw bytes */
dev_err_t svc_shell_execute_line(char *line);           /* Parse and execute a line */
dev_err_t svc_shell_find_command(const char *name, const svc_shell_cmd_t **cmd); /* Lookup */
bool     svc_shell_is_initialized(void);                /* Check state */
dev_err_t svc_shell_register_command(const svc_shell_cmd_t *cmd);        /* Runtime register */
dev_err_t svc_shell_unregister_command(const char *name);                 /* Runtime unregister */
dev_err_t svc_shell_unregister_all_runtime_commands(void);                /* Clear runtime */
uint16_t svc_shell_get_runtime_command_count(void);                         /* Count runtime */
```
...
At init time, `svc_shell_init()` validates the command table:
- Rejects more than `SVC_SHELL_CFG_MAX_COMMANDS` entries
- Rejects duplicate command names
- Rejects NULL command names
- Rejects NULL function pointers

If validation fails, `svc_shell_init()` returns `DEV_ERR_CONFIG`.
...
target_link_libraries(${PROJECT_NAME} svc_shell)
```
...
| UART auto-init | `svc_shell_init()` calls `dev_uart_init()` for you |
| `svc_shell_handle()` returns quickly | Non-blocking — safe in main loop or low-priority task |
